...
S\arrow[r, "\iota"]\arrow[rd, "f"'] & F\arrow[d, "\exists!\phi", dashed]\\
& G
\end{tikzcd}}
...
\adjustbox{scale=1.1,center}{\begin{tikzcd}
G_i\arrow[r, "\iota_i"]\arrow[rd, "\phi_i"'] & \ast_iG_i\arrow[d, "\exists!\ast_i\phi_i"]\\
& H
\end{tikzcd}} \tcbline
\begin{proof}
...
\adjustbox{scale=1.1,center}{\begin{tikzcd}
B\arrow[r, "\iota"]\arrow[rd, "f"'] & G\arrow[d, "\exists !F", dashed]\\
& A
\end{tikzcd}}
\end{thm}
...
The maximal power of the prime $p$ that divides $\abs{G}$ is also called the $p$-part of $G$. This is denoted as $\abs{G}_p$. Before we prove the main theorems of the section, we need some lemmas. 

\begin{lmm}{}{} Let $P\in\text{Syl}_p(G)$ be a Sylow $p$ subgroup of a group $G$. If $Q$ is any $p$ subgroup of $G$, then $Q\cap N_G(P)=Q\cap P$
\end{lmm}
...
We resume the proof of the second Sylow theorem. Since we know that $\abs{X}\equiv m\;(\bmod\;p)$, and that $$\abs{X}\equiv\sum_{i=1}^r\abs{O_{T_i}}\;(\bmod\;p)$$ Since $\abs{\text{Orb}_G(T_i)}=[G:\text{Stab}_G(T_i)]=\abs{G}/\abs{T_i}=m$ by claim 1, and $t=\abs{\text{Syl}_p(G)}$ by claim 2. From the above sum we deduce that $$m\equiv mn_p(G)\;(\bmod\;p)$$ Since we also have that $\gcd(m,p)=1$, we can cancel $m$ on both sides to get $n_p(G)\equiv1\;(\bmod\;p)$. This also proves the first Sylow theorem. 
\end{proof}
\end{thm}
...
Let $p=3$ or $5$ such that $p$ divides $\abs{N}$. Then we know that $p$ does not divide $[G:N]$ so by the above theorem we deduce that $F_p(G)=F_p(N)$. If $p=5$ then $F_p(N)=24$ and thus $\abs{N}\geq 25$. Since $\abs{N}|60$ this implies that $\abs{N}=30$. Then there are only distinct cosets of $N$ in $G$ and thus $N$ is normal. If $p=3$ then $F_p(N)=20$ and thus $\abs{N}\geq 21$. Since $\abs{N}|60$ we thus have $\abs{N}=30$. \\~\\
Case 2: Neither $3$ or $5$ divides $\abs{N}$. Then $\abs{N}$ divides $4$ by Lagrange's theorem. By Cauchy's theorem, there exists $x\in N$ of order $2$. Thus using the above corollary, we have that $4=\abs{N}\geq\text{Cl}(x)=15$ which is a contradiction. 
\end{proof}
\end{prp}
...
\begin{prp}{}{} Let $G$ be a group such that $g^2=1$ for all $g\in G$. Then $G$ is abelian. 
\end{prp}
...
\begin{lmm}{Fitting's Lemma}{} Let $G$ be a finite group. Suppose that $K\trianglelefteq G$ is abelian with odd order $\frac{\abs{G}}{2}$. Let $H=\langle x\rangle\in\text{Syl}_2(G)$. Let $k\in K$. Define $$[K,x]=\langle[v,x]|v\in K\rangle$$ Then the following are true. 
\begin{itemize}
...
\begin{defn}{Dicyclic Group of Order $12$}{} Let $C_4=\langle h\rangle$. Define $\phi:C_4\to\text{Aut}(C_3)$ to be the inversion homomorphism for. Define the dicyclic group of order $12$ to be $$\text{Dic}_{12}=C_4\ltimes_\phi K$$ 
\end{defn}

\begin{prp}{}{} Let $G$ be a group of order $12$. Then $G$ is isomorphic to one of the following groups: 
\begin{itemize}
