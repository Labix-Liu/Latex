...
\begin{lmm}{}{} Let $X$ be a space and let $A\subseteq X$ be a subspace. Let $G$ be an abelian group. Then the dual of the short exact sequence of chain complex \\~\\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
0\arrow[r] & C_\bullet(A)\arrow[r, "\iota"] & C_\bullet(X)\arrow[r, "j"] & C_\bullet(X,A)\arrow[r] & 0
\end{tikzcd}}\\~\\ given explicitly by \\~\\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	...
	\arrow[from=1-4, to=1-5]
\end{tikzcd}}\\~\\
is also a short exact sequence of chain complexes. \tcbline
\begin{proof}
Notice that in the original short exact sequence, the sequence \\~\\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	...
	\arrow[from=1-4, to=1-5]
\end{tikzcd}}\\~\\
is split exact. Since the Hom functor preserves split exact sequence, we have that the following sequence given by \\~\\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	0 & {C^n(X,A;G)} & {C^n(X)} & {C^n(A)} & 0
	\arrow[from=1-1, to=1-2]
	\arrow["{j^\ast}", from=1-2, to=1-3]
	\arrow["{i^\ast}", from=1-3, to=1-4]
	\arrow[from=1-4, to=1-5]
\end{tikzcd}}\\~\\
is also split exact. Therefore in particular the short exact sequence of chain complexes is short exact. 
\end{proof}
\end{lmm}
...
\adjustbox{scale=1.0,center}{\begin{tikzcd}
\cdots\arrow[r] & H^{n-1}(A)\arrow[r, "\partial"] & H^n(X,A)\arrow[r, "j^\ast"] & H^n(X)\arrow[r, "i^\ast"] & H^n(A)\arrow[r] & \cdots
\end{tikzcd}}\\~\\
where $\iota:A\to X$ is the inclusion map and $j:X\to X\setminus A$ is the quotient map. Moreover, the connecting homomorphism $\partial:H_{n-1}(A)\to H_n(X,A)$ is defined by ?????. \tcbline
\begin{proof}
Notice that \\~\\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	0 & {C^\bullet(X,A;G)} & {C^\bullet(X)} & {C^\bullet(A)} & 0
