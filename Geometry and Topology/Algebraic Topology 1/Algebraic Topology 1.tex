...
\item Given the $n-1$ skeleton $X^{n-1}$, a collection of closed $n$-dimensional disks $\{D_{\alpha}^n|\alpha\in I_n\}$ such that $D_\alpha^n\cong B_1(0)\subseteq\R^n$ with $S_\alpha^{n-1}=\partial D_\alpha^n$ and a collection of attaching maps $\varphi_\alpha:S_\alpha^{n-1}\to X^{n-1}$. Now define $$X^n=\left(X^{n-1}\sqcup\bigsqcup_{\alpha\in I_n}D_\alpha^n\right)/\sim$$ where $\sim$ is the equivalence relation $x\sim\varphi_\alpha(x)$ for all $x\in S_\alpha^{n-1}$ (The unions in $X$ are all disjoint)
...
\begin{defn}{Characteristic Maps}{} Let $X$ be a CW-complex. Let $D_\alpha^n$ be a closed cell of $X$. Define the characteristic map of $D_\alpha^n$ to be the map $$\Psi_\alpha^n:D_\alpha^n\to X$$ given by the following diagram \\~\\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
...
\begin{prp}{}{} Let $X$ be a CW-complex of finite dimension. Then the weak topology on $X$ is equal to the quotient topology on $X$. 
\end{prp}
...
\begin{prp}{}{} A compact topological subspace of a CW complex $X$ is contained in a finite subcomplex. 
\end{prp}
